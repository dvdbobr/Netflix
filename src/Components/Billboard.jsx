...
export default function Billboard(props) {
  const [movieIndex] = useState(
    props.originals[Math.floor(Math.random() * props.originals.length)]
  );
  ...
  const showTrailer = async (movie) => {
    let trailerurl = await axios.get(
      `${endpoint}/tv/${movie.id}/videos?api_key=a3d71a761a7bb30717e08b95a73a97c4`
    );
  ...
  return (
    <div
      ...
      style={{
        background: `url(${
          props.imgEndpoint + movieIndex.backdrop_path
    ...
    >
      <div className="billboardDetails">
        ...
        <div className="billboardButtons">
          <button className="billBoardPlay">
            <a
              style={{ color: "black" }}
          ...
          <button className="billBoardInfo">
            <BiInfoCircle size={34} /> More Info
          </button>
        </div>
      </div>
    </div>
